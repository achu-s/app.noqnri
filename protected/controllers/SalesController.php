...
class SalesController extends Controller
{
	...
	public function actionView($id)
	{
	    ...
	    if(Yii::app()->user->userType=="Admin" || (Yii::app()->user->userType=="Partner" && $model->parent_id==Yii::app()->user->partner)){
	...
	{
	    if(Yii::app()->user->userType=="Admin" || (Yii::app()->user->userType=="Partner" && $id==Yii::app()->user->partner)){
	        $len=16;
	        $last=-1;
	        for ($i=0;$i<$len;$i++)
	        {
	...
	{
	    $model=$this->loadModel($id);
	    if(Yii::app()->user->userType=="Admin" || (Yii::app()->user->partner==$model->parent_id)){
        			        ...
        			        if($old_pic!=NULL && file_exists($old_path)){
        			         ...
        			         if($imageData->save()){
        			             ...
        			             if(Yii::app()->user->id==$id){
        			                 ...
        			                 $this->redirect(Yii::app()->baseURL.'/sales/index/'.$model->parent_id);
        			             }
        			         }else{
        			             $transaction->rollBack();
